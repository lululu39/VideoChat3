...
**Status:** Group-4 execution was rejected after the real 2K relaunch OOMed before step 1. The launcher now uses layer-major group 1 with targeted compiled memory apply, exact NS5 backward, and state-only recurrent clipping; no checkpoint exists yet.
...
- Hardware and packing: 8xH100, global batch 16, 2K sample/pack length, TimeLens 2 FPS, 64-448 frames rounded to four, total-pixel budget 14,680,064, and R4 token-select placeholders/timestamps. The selected group-1 path retains packed cross-chunk FlashAttention and whole-layer temporal-scan activation checkpointing. Per-chunk memory apply is compiled; the 1152-wide NS5 update remains eager because that is faster on H100. The first 8K attempt packed about 8.5 full videos into each sequence and exhausted every 80GB H100 before step 1 because LACT still processes all original chunks; 2K restores roughly two videos per pack without changing R4 semantics, source data, optimizer, LR, or trainable scope.
...
The real group-1 FSDP/data smoke completed three optimizer steps and was then stopped without a checkpoint. Stable steps 2/3 take `95.07/96.53s` (`95.80s` mean); the largest observed per-rank allocated peak is `69.23 GB`. This is effectively unchanged from the old group-1 step-2 timing of `95.67s`, so targeted compiled memory apply plus disabling NS5 ratio clipping does not materially improve end-to-end throughput. By comparison, v10 Base R4 averages `26.30s/step`, peaks at `48.14 GB`, and needs 93 rather than 455 steps: LACT group 1 is `3.64x` slower per optimizer step and has `4.89x` as many steps because its all-chunk vision execution cannot fit 8K packing. At the measured stable rate LACT takes about `12.1h` before startup/checkpoint overhead, versus v10's actual 44 minutes (`~16.5x` end-to-end). The speed smoke used the real TimeLens manifest, 8xH100 FSDP, global batch 16, 2K packing, and W&B disabled; log: `xtuner-videochat3/work_dir/stage3/vc3-lact-group1-compile-speed-smoke/torchrun_logs/training_20260901_162716_datava270000004.log`.
