...
**Status:** Prepared for the user-authorized W&B launch on 2026-09-09; validate startup below. Use the entire prepared QA training split for one epoch, with optional user-directed early stopping. There is no 200-step training cap and no automatic loss-based stop.
...
- Expected artifact: `/mnt/localssd/VideoChat3/training/vc3-lact-l16-delta-3drope-parallel-alltokens-vitfwproj-llava0to30-qa495013-4xh100-gb16-f64-s8k-lr2e5-v35/<timestamp>/hf-200` first periodic export; `hf-5158` on normal full completion. Save a usable adapted checkpoint before a later manual stop and TimeLens transfer; use a new TimeLens run/optimizer/dataloader with the same no-query/all-token architecture.
