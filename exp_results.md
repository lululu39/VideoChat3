...
Linear cross-layer batching follows the VideoMamba chunk-major topology and folds four layers into one batch of 64 layer-head state matrices; group-1/group-4 FP32 parity is within `8.35e-7` forward and `1.91e-6` input-gradient error, with or without state clipping. To avoid the old per-chunk checkpoint-state explosion, Linear cross-layer mode checkpoints each complete four-layer temporal scan. Nevertheless, FSDP 8K group 4 still OOMs during this outer checkpoint's backward recomputation at roughly `79.15-79.17 GB` process use / `76.97-76.99 GB` PyTorch allocation, failing on another `14-20 MB`. No optimizer step or meaningful steady power window completes; the initial roughly `130W` sample is compilation/idle time and cannot establish a utilization gain. Log: `xtuner-videochat3/work_dir/stage3/vc3-lact-linear16-delta-cross4-fsdp-s8k-power-smoke/torchrun_logs/training_20260901_215752_datava270000004.log`.
