...
**Status:** Stopped after the optimized real 2K/group-4 relaunch OOMed before step 1. No checkpoint exists. The original 8K launch also OOMed before step 1; the original unoptimized 2K run reached step 2/455 with a measured 11h47m ETA, then was stopped for diagnosis.
...
- Hardware and packing: 8xH100, global batch 16, 2K sample/pack length, TimeLens 2 FPS, 64-448 frames rounded to four, total-pixel budget 14,680,064, and R4 token-select placeholders/timestamps. FW updates are grouped across four consecutive vision layers and compiled as one layer-batched update; per-chunk memory apply is also compiled, while packed attention and the 1152-wide NS5 update use the fastest measured execution form. The first 8K attempt packed about 8.5 full videos into each sequence and exhausted every 80GB H100 before step 1 because LACT still processes all original chunks; 2K restores roughly two videos per pack without changing R4 semantics, source data, optimizer, LR, or trainable scope.
