...
Evaluation validation: all 4,279 official MP4s / 9,404 queries validated (75,096,716,925 video bytes); preparation summary at dataset-root `timelens_bench_prepare_summary.json`. Local AutoConfig/AutoProcessor both load factor 16 / `select_last`; generation options and all three dataset configs exactly match v26. The user-requested eight-rank restart preserves all 723 four-rank Charades predictions from `T20260911_G8bd175f0` and assigns the remaining 2,640 queries evenly (330/rank); all eight ranks have produced new native answers without OOM or failed generation. Unrelated GPU processes are untouched. Current artifact directory `/mnt/localssd/VideoChat3/eval/videochat3-v38-hf261-timelens-bench/VideoChat3-4B-LACT-v38-hf261/T20260911_Gde7549dd`; detached session `vc3-v38-hf261-bench-8gpu`, log `/mnt/localssd/VideoChat3/setup/v38_hf261_bench_8gpu.log`. Training W&B state is `killed` with the explicit user-stop / retained-checkpoint note.
...
- Stabilization: only rho-1 state-adjoint clipping remains enabled, followed by true FSDP global gradient clipping at 1.0. NS5 ratio clipping is disabled. Each NS5 call is nevertheless locally recomputed during backward to return its exact, unclipped VJP without retaining every update's ordinary NS5 graph across the whole-layer checkpoint recomputation; this is activation checkpointing, not gradient clipping.
...
The explored path adds configurable four-layer FW update batching and targeted `torch.compile`. CPU FP32 parity on two videos with full/tail chunks gives maximum forward and input-gradient differences of `1.10e-6/1.19e-6`; the complete state-adjoint clip remains finite. On a real 27-layer H100 training forward+backward at 32 frames, layer-batch 4 takes `2.11s` versus `2.49s` for layer-major (`15.0%` faster), at the cost of `16.88` versus `6.52 GiB` standalone peak memory. Inference alone is `5.8%` slower because chunk-major execution gives up packed cross-chunk FlashAttention. Neither group 4 nor per-chunk compile is selected for the active training path. Benchmarks are under `/mnt/localssd/VideoChat3/benchmark_h100_lact_*compile*.json` and `/mnt/localssd/VideoChat3/benchmark_h100_lact_training_group_compare.json`.
...
The real compiled group-1 FSDP/data smoke completed three optimizer steps and was then stopped without a checkpoint. Stable steps 2/3 take `95.07/96.53s` (`95.80s` mean); the largest observed per-rank allocated peak is `69.23 GB`. An eager rerun with NS5 local recomputation still disabled reproduced the high step-1 range (`55.87-62.45 GB`, versus the old `37.45-39.14 GB`), proving that ordinary exact NS5 autograd—not compile—caused the large memory regression. Per-chunk compile was still removed because it provided no end-to-end speed gain. The active path now separates semantics from memory: NS5 gradients remain exact and unclipped, while each NS5 VJP is locally recomputed to avoid retaining all recurrent NS5 graphs. By comparison, v10 Base R4 averages `26.30s/step`, peaks at `48.14 GB`, and needs 93 rather than 455 steps: LACT group 1 is about `3.64x` slower per optimizer step and has `4.89x` as many steps because its all-chunk vision execution cannot fit 8K packing. At the measured stable rate LACT takes about `12.1h` before startup/checkpoint overhead, versus v10's actual 44 minutes (`~16.5x` end-to-end). Smoke logs are under `xtuner-videochat3/work_dir/stage3/vc3-lact-group1-*-smoke/torchrun_logs/`.

The final exact-recompute smoke verifies the selected behavior on the identical first two optimizer batches. Step 1 uses `37.45-39.14 GB`; stable step 2 uses `37.88-45.07 GB` and `95.41-95.44s` across ranks. These memory values exactly recover the old bounded-NS5 run, while the global step-2 gradient norm changes from `3.34759665` to `3.34858394`, confirming that the active backward is not the old clipped gradient. The smoke stopped after step 2 with no checkpoint; native log: `xtuner-videochat3/work_dir/stage3/vc3-lact-group1-exact-recompute-smoke/torchrun_logs/training_20260901_164914_datava270000004.log`.

An explicit `inner_optim="muon"|"sgd"` option now permits an isolated raw-SGD state update without changing the error, learned token-wise rates, FP32 master state, or runtime normalization. SGD is bitwise identical to the prior `muon_update_steps=0` behavior. On the real 27-layer/1152-width vision tower, SGD improves 64/224-frame inference from `0.875/3.135s` to `0.576/2.046s` (`1.52x/1.53x`) and a 32-frame checkpointed forward+backward from `2.624s` to `1.697s` (`1.55x`), with no memory increase. This is only a compute result: at the existing inner LR `0.01`, one representative 1024-token real-width update changes the normalized runtime state by `1.50%` under SGD versus `62.99%` under Muon, a roughly `42x` magnitude difference. Benchmarks: `/mnt/localssd/VideoChat3/benchmark_h100_lact_inner_optim.json`, `/mnt/localssd/VideoChat3/benchmark_h100_lact_inner_optim_training.json`, and `/mnt/localssd/VideoChat3/benchmark_h100_lact_inner_optim_update_scale.json`.

The real 8xH100 FSDP/TimeLens SGD smoke confirms a smaller end-to-end gain. On the identical stable second batch, Muon exact-recompute takes `95.41-95.44s` with a `45.07 GB` maximum, while SGD takes `71.14-71.27s` with a `40.59 GB` maximum: `1.34x` faster and about `10%` less peak memory. At 455 steps this projects to roughly `9.0h` before startup/checkpoint overhead, still about `12x` v10 Base R4's actual 44 minutes. Step 1 has zero outer LR and step 2 therefore still evaluates initialization, so this smoke measures systems performance rather than SGD learning quality. It stopped after step 2 without a checkpoint; log: `xtuner-videochat3/work_dir/stage3/vc3-lact-group1-sgd-speed-smoke/torchrun_logs/training_20260901_172115_datava270000004.log`. The v11 launcher remains Muon by default until an SGD inner-LR/training decision is made.

### State-Clip Controlled Smoke

Two short real TimeLens runs compare Muon with no FW ratio clip (`clip_ns_grad_ratio=False`, `clip_state_grad_ratio=False`) against the selected default (`clip_ns_grad_ratio=False`, `clip_state_grad_ratio=True`). Both use exact local NS5 recomputation, group 1, R4 selection, 2K packing, identical initialization/data order, 8xH100 FSDP, and the framework's global 1.0 clip. Logged `grad_norm` is the true pre-clip global norm. W&B is disabled and neither run will retain a checkpoint.

The no-FW-clip run reports pre-clip global norms `3.6351`, `3.3454`, then `inf` at step 3 on all eight ranks; XTuner correctly rejects the invalid norm and skips that optimizer step. The state-only default reports `3.5947`, `3.3439`, and finite `3.8178` on the corresponding first three steps. Thus the uncontrolled recurrence can look benign for two steps and fail immediately after the first nonzero outer update, while state-adjoint clipping prevents the same failure without NS5 gradient clipping. State clipping costs about 6-7% here (`88.62/90.18s` unclipped versus `94.41/95.86s` clipped on steps 2/3) and does not increase the `45.07 GB` observed maximum. Both runs stopped after step 3 with no checkpoint. Logs: `xtuner-videochat3/work_dir/stage3/vc3-lact-timelens-fwclip-off-grad-smoke/torchrun_logs/training_20260901_173044_datava270000004.log` and `xtuner-videochat3/work_dir/stage3/vc3-lact-timelens-stateclip-on-grad-smoke/torchrun_logs/training_20260901_173750_datava270000004.log`.

### Chunk-Level Linear Memory

`memory_type="linear"` adds a single multi-head linear state per layer while preserving private attention-share-initialized Q/K/V/O, apply-then-update semantics, one update per four-frame chunk, continuous per-video state, final-update skipping, zero residual gate, and optional state-adjoint clipping. `inner_optim="muon"` applies NS5 to the same chunk prediction gradient; `inner_optim="delta"` applies it raw. For SwiGLU memory, `delta` is the canonical name for the existing raw-gradient path and legacy `sgd` remains an exact alias. Linear memory currently uses group 1 only.

Zero-gate output parity between SwiGLU and linear memory is exact, and linear `delta` equals legacy `sgd` bitwise. With 16 linear heads, FW parameters fall from `358.47M` to `143.89M`. At 224 frames, vision inference is `3.162s` (SwiGLU+Muon), `2.066s` (SwiGLU+Delta), `1.662s` (Linear+Muon), and `1.122s` (Linear+Delta); Linear+Delta is `2.82x` faster than the current memory. For a real-width 32-frame checkpointed forward+backward the corresponding medians are `2.627/1.681/1.625/1.070s`; Linear+Delta is `2.46x` faster and reduces peak total allocation from `5.45` to `3.06 GiB` (`2.36` to `0.80 GiB` working allocation). These are systems/parity results, not learning-quality evidence. Artifacts: `/mnt/localssd/VideoChat3/benchmark_h100_lact_memory_inner_grid.json` and `/mnt/localssd/VideoChat3/benchmark_h100_lact_memory_inner_grid_training.json`.

Real 8xH100 FSDP/TimeLens smokes validate both linear variants and initialization loading. On the identical stable second batch, Linear+Muon takes `63.80-63.87s`, peaks at `18.41 GB`, and has pre-clip norm `3.3619`; Linear+Delta takes `43.08-43.36s`, peaks at `18.39 GB`, and has norm `3.3424`. Relative to SwiGLU+Muon at `95.41-95.44s / 45.07 GB`, Linear+Muon is `1.49x` faster and Linear+Delta is `2.20x` faster, while both cut peak allocation by about `59%`. Delta contributes a further `1.47x` over Muon on the same linear memory. At 455 2K-packed steps, Linear+Delta projects to roughly `5.5h` before startup/checkpoint overhead; the much smaller state may permit a later 8K packing test, but that was not attempted here. Both smokes stopped after step 2 without checkpoints. Logs: `xtuner-videochat3/work_dir/stage3/vc3-lact-linear16-muon-fsdp-smoke/torchrun_logs/training_20260901_183452_datava270000004.log` and `xtuner-videochat3/work_dir/stage3/vc3-lact-linear16-delta-fsdp-smoke/torchrun_logs/training_20260901_183939_datava270000004.log`.
...
- Data: seed-42 12,624-row TimeLens random-half manifest over 8,985 videos, identical to v11/v10.
...
- Expected artifact: `xtuner-videochat3/work_dir/stage3/vc3-4b-lact-linear16-delta-lastchunk-fwproj-timelens-rand12624-8xh100-gb16-video2fps-f448-s1k-lr2e5-nofwclip-v13/<timestamp>/hf-114`.
...
The training config now supports `VIDEOCHAT3_PARALLEL_STRATEGY=ddp`. Its backend is FSDP2 HSDP with shard size 1: all parameters are fully replicated on each rank and gradients synchronize over the eight-rank replicate dimension, eliminating parameter sharding/all-gathers while preserving XTuner mixed precision, activation checkpointing, DCP, and grad-norm behavior. This is functionally DDP; a real power/utilization comparison is pending below.

Replicated-DP does not improve Linear+Delta throughput. At 2K it takes `39.69-39.75s/step`, effectively identical to FSDP's `39.27-40.31s`, while peak allocation rises from `18.39` to `35.23 GB`. A 30-second compute-window sample gives `217-235W` average and `288-309W` peak per GPU, with `53-80%` average SM busy. At 8K, replicated-DP OOMs in the first vision-MLP checkpoint recomputation during backward: PyTorch has `72.29-74.27 GB` allocated and `76.45-78.45 GB` total process use, then fails requesting another `3.00-3.22 GiB`; instantaneous device use reaches `80.95 GB`. Thus DDP neither raises sustained power materially nor supports 8K. Logs: `xtuner-videochat3/work_dir/stage3/vc3-lact-linear16-delta-ddp-s2k-power-smoke/torchrun_logs/training_20260901_212421_datava270000004.log` and `xtuner-videochat3/work_dir/stage3/vc3-lact-linear16-delta-ddp-s8k-power-smoke/torchrun_logs/training_20260901_212920_datava270000004.log`.

The historical v10 logs contain no power telemetry, so an exact Base R4 8K replay was sampled. It reproduces `25.47-26.34s` stable steps and approximately the historical memory (`48.97 GB` observed versus `48.14 GB`). During the main dense compute phase, all eight H100s simultaneously draw `639-683W`; a 10-second interval spanning phase transitions averages `288-296W` with per-GPU peaks `678-693W`. This confirms Base R4 has genuine near-limit dense-GEMM bursts, whereas Linear+Delta's recurrent path remains below roughly 300W. Replay log: `xtuner-videochat3/work_dir/stage3/vc3-base-r4-v10-power-replay/torchrun_logs/training_20260901_214145_datava270000004.log`.

The first Linear cross-layer implementation folded four layers into one update batch but retained a complete temporal scan inside each layer group; it was therefore not the strict VideoMamba chunk-major topology. It had group-1/group-4 FP32 parity within `8.35e-7` forward and `1.91e-6` input-gradient error, but FSDP 8K OOMed during whole-group checkpoint recomputation at roughly `79.15-79.17 GB` process use / `76.97-76.99 GB` PyTorch allocation. This superseded result completed no optimizer step. Log: `xtuner-videochat3/work_dir/stage3/vc3-lact-linear16-delta-cross4-fsdp-s8k-power-smoke/torchrun_logs/training_20260901_215752_datava270000004.log`.

The corrected experimental Linear path uses the strict dependency order: one four-frame clip traverses all 27 layers, then independent layer/head state updates are batched, before the next clip. Group 1 versus group 4/27 CPU checks agree within `9.69e-7` forward and `1.91e-6` input-gradient error. Its first checkpoint version covered the complete 27-layer clip plus update. A real group-27 Linear16+Delta FSDP 8K run reduced active process memory to about `21-22 GB`, proving that only one clip's activations were live, but it did not finish one optimizer step after more than ten minutes from `grad_accumulation_steps` startup and was stopped. Compute samples were generally only about `120-190W/GPU` (one rank intermittently reached about `261W`) despite nominally high SM busy. Log: `xtuner-videochat3/work_dir/stage3/vc3-lact-linear16-delta-cross27-fsdp-s8k-power-smoke2/torchrun_logs/training_20260901_222001_datava270000004.log`.

The checkpoint boundary was then changed to match VideoMamba exactly: every layer's single-clip attention/memory/slow-MLP block is checkpointed independently, every batched state update has a separate checkpoint, and layer update parameters are stacked once outside the temporal loop. A two-layer/two-video CPU regression gives bitwise-equal forward, input gradients, and all 44 parameter-gradient tensors with checkpointing enabled versus eager. The same real G27/FSDP/8K workload nevertheless remains unsuitable: during the first minute after `grad_accumulation_steps`, retained per-layer clip boundaries raised device use progressively from about `11 GB` to at least `37 GB`, while sampled power was mostly `120-160W` and SM busy mostly `0-17%`; no optimizer step completed before the user stopped it. Thus reference checkpointing trades whole-clip recomputation for retained layer boundaries but does not improve GEMM granularity or MFU in the current batch-size-one clip scheduler. Log: `xtuner-videochat3/work_dir/stage3/vc3-lact-linear16-delta-cross27-layerckpt-fsdp-s8k-power-smoke/torchrun_logs/training_20260901_224530_datava270000004.log`.

This does not contradict VideoMamba's cross-layer result. Its controlled F64 benchmark found only a `4.97%` private-projection speedup at the G4 optimum (`1.6941 -> 1.6139s`) while peak allocation rose `14.25 -> 26.54 GiB`; G8 already regressed and G16/G32 were slower. Each VideoMamba chunk remains a regular `B8 x 8 tubelets x 197 tokens = 12,608`-row dense batch, with exactly eight chunks and compiled per-layer chunk apply/update. VideoChat3 currently loops over packed videos, initializes state with batch size one, and sends one variable four-frame clip at a time through the 27 layers; at the 224-pixel cap a square clip is only `4 x 16 x 16 = 1,024` rows, while the average video has 58.5 clips. It therefore discards the all-video/all-clip FlashAttention and slow-MLP packing that makes group 1 efficient. Moreover, this test batches cheap single-state Delta updates, not VideoMamba's three-matrix SwiGLU plus five-step Muon update, so there is much less update overhead to amortize. Group 27 also greatly overshoots VideoMamba's measured G4 optimum. Keep group 1 as the runnable production path. The plausible MFU path is not more layer batching: retain layer-major packed attention/slow-MLP and, within each layer's FW scan, batch the same temporal index across all active videos in the pack, bucketed by clip token length. Only after that supplies a real video batch should a small G4 layer-update sweep be reconsidered.
...
**Status:** Training completed at step 114/114. Initialize from `/mnt/localssd/VideoChat3/VideoChat3-4B`; train all `416,870,640` ViT plus `33,039,616` projector parameters with the v10 `2e-5 -> 1e-6` schedule, global clip 1.0, frozen LM, Base `video_last`, 8xH100 FSDP, global batch 16, and 1K packs. First/final loss is `0.8002/0.3899`; first/last-20 mean is `0.5190/0.3863`; grad norm mean/median/max is `0.467/0.213/8.287`; stable step median is 22.13 s and peak allocation is 53.93 GB. LM is bitwise unchanged; ViT attention/MLP/other and projector relative L2 deltas are `0.484%/0.219%/0.0129%/1.129%`. Checkpoint: `20260902060900/hf-114`; inspection: `20260902060900/checkpoint_inspection.json`; W&B: [`v14`](https://wandb.ai/LVSM-Experiment/videochat3/runs/vc3-4b-base-video-last-vitproj-timelens-rand12624-8xh100-gb16-video2fps-f448-s1k-lr2e5-v14).

TimeLens-Bench completed on 9,404/9,404 queries: Charades R1@0.3/0.5/0.7/mIoU `17.54/10.05/3.57/11.80%`; ActivityNet `9.60/5.20/2.04/7.74%`; QVHighlights `2.14/1.23/0.52/3.06%`. Native artifacts: `/mnt/localssd/VideoChat3/eval/videochat3-v14-timelens-bench/VideoChat3-4B-Base-video-last-v14/T20260902_G82fb1873`.
...
**Status:** Stopped by the user at step 25/114. No checkpoint or resume state exists because the first save interval is step 100; do not resume this run.
...
- Memory/update: same as v13, `memory_type=linear`, 16 heads, `inner_optim=delta`, group 1, continuous per-video state, apply-then-update per four-frame chunk, and final update skipped. Fast Q/K use `lact_3d_rope=True`: T is continuous across chunks within each video and H/W are patch coordinates. The residual uses `lact_gate="tanh"`, which bounds the effective gate but preserves its zero initialization and near-zero first-order scale.
...
- Training W&B: [`v16`](https://wandb.ai/LVSM-Experiment/videochat3/runs/vc3-4b-lact-linear16-delta-3drope-tanh-lastchunk-fwproj-timelens-rand12624-8xh100-gb16-video2fps-f448-s1k-lr2e5-nofwclip-v16).
...
The tanh-gate restart uses fresh run ID `vc3-4b-lact-linear16-delta-3drope-tanh-lastchunk-fwproj-timelens-rand12624-8xh100-gb16-video2fps-f448-s1k-lr2e5-nofwclip-v16` because W&B would not reuse the deleted prior ID. All ranks explicitly report `VIDEOCHAT3_LACT_GATE=tanh` and `VIDEOCHAT3_LACT_3D_ROPE=1`; run directory `20260903155802`, native log `torchrun_logs/training_20260903_155745_datava270000004.log`. Through the common first 23 steps, v13/v16 loss correlation is `0.999989`, mean absolute loss difference is `5.13e-4`, and v16's signed mean improvement is only `3.42e-5` (`0.006%`), confirming that zero-init tanh gating remains effectively linear and suppresses the 3D-RoPE branch.
...
- Data: same seed-42 12,624-row TimeLens random-half manifest over 8,985 videos as v13/v16.
...
- Memory/update: `memory_type=linear`, 16 heads, `inner_optim=delta`, group 1, continuous per-video state, final update skipped, and global-frame T plus patch-coordinate H/W 3D RoPE on fast Q/K. Relative to v16, `lact_gate="linear"` and `lact_gate_init=0.5` replace zero-init tanh gating.
...
**Status:** Stopped by the user after step 15/114; no checkpoint exists and this run must not be resumed or evaluated.
...
**Status:** Training completed at step 413/413; checkpoint diagnostics and native TimeLens-Bench evaluation are complete.
...
- Trainable scope: all `143,888,256` LACT-added parameters including the 1,152-element shared query and gates plus all `33,039,616` projector parameters (`176,927,872` total); original ViT and 4B LM frozen.
...
- Stabilization: no FW ratio clip or NS5; XTuner global gradient clip remains 1.0.
- Hardware/batch/sequence: 8xH100 ordinary FSDP, global batch 16, 1K sample/pack length, 2 FPS, 64-448 frames, total-pixel budget 14,680,064, one retained LM visual token per four-frame chunk, 6,593 packs, and 413 optimizer steps.
...
**Status:** Completed on all 9,404 queries with no missing predictions. Fixed native generation/scoring protocol matches v17-v19: 2 FPS, up to 448 frames, 224px/14,680,064-total-pixel budget.

| Subset | Base video-last R1@0.3 | v23 R1@0.3 | Delta | Base R1@0.5 | v23 R1@0.5 | Delta | Base R1@0.7 | v23 R1@0.7 | Delta | Base mIoU | v23 mIoU | Delta |
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| Charades-TimeLens | `17.54%` | `17.93%` | `+0.39` | `10.05%` | `10.65%` | `+0.60` | `3.57%` | `4.28%` | `+0.71` | `11.80%` | `12.51%` | `+0.71` |
| ActivityNet-TimeLens | `9.60%` | `10.56%` | `+0.96` | `5.20%` | `6.58%` | `+1.38` | `2.04%` | `3.84%` | `+1.80` | `7.74%` | `8.48%` | `+0.74` |
| QVHighlights-TimeLens | `2.14%` | `7.07%` | `+4.93` | `1.23%` | `4.74%` | `+3.51` | `0.52%` | `3.11%` | `+2.59` | `3.06%` | `6.60%` | `+3.54` |
...
- Data: identical seed-42 12,624-row TimeLens random-half manifest and exact v24-v26 4,401-pack query-aware cache, preserving samples and order.
- Trainable scope: identical to v26, all `416,870,640` original ViT parameters, all `143,905,536` LACT-added parameters including query/gates, and all `33,039,616` projector parameters (`593,815,792` total); only the 4B LM is frozen.
- Memory/update: identical Linear16+Delta group-1 continuous state, fast-Q/K 3D RoPE, Base-R4-budget per-chunk queries with identity query RoPE and no query writes, apply-then-update, final update skip, and zero-initialized linear gates. Only `fw_order="parallel" -> "serial"` changes, so the FW branch consumes the post-attention hidden state.
...
- Activation memory: `VIDEOCHAT3_VISION_ACTIVATION_OFFLOAD=1` wraps each checkpointed vision block with PyTorch CPU activation offload. Saved block inputs move to host memory; the forward function and backward derivatives are unchanged. Full layer-major checkpointing is required; parameter offload remains disabled.
- Hardware/batch/sequence: identical to v19, 8xH100 ordinary FSDP, global batch 16, 1K sample/pack length, 2 FPS, 64-448 frames, total-pixel budget 14,680,064, 1,815 packs, and 114 optimizer steps. Launch only after confirming the GPUs are idle; keep `GPU_EXCLUSIVE=0` so no watchdog terminates unrelated jobs. The offload restart must reproduce steps 1-2 and pass the previously failing step 3 before startup is accepted.
- Training W&B: [`v29 activation-offload restart`](https://wandb.ai/LVSM-Experiment/videochat3/runs/vc3-lact-l16-delta-3drope-parallel-gate0-lastchunk-vitfwproj-timelens-r12624-8xh100-gb16-f448-s1k-lr2e5-actoffload-clean-v29).
...
- Expected artifact: `/mnt/localssd/VideoChat3/training/vc3-lact-l16-delta-3drope-parallel-gate0-lastchunk-vitfwproj-timelens-r12624-8xh100-gb16-f448-s1k-lr2e5-actoffload-clean-v29/20260907013030/hf-114`; the corresponding XTuner work directory is a symlink to this SSD run root.

Clean startup: steps 1-3 CE is `0.80019104/0.78642583/0.78507501`, with finite global pre-clip norms `2.56800675/2.71005774/2.75986886`. The first two CE values exactly reproduce the original v29 attempt; corresponding norms differ by less than `0.06%`, so distributed gradients are not claimed bitwise identical. Maximum rank allocated/reserved memory on steps 1/2/3 is `50.42/53.73`, `52.88/56.33`, and `53.91/57.33 GB`; no OOM or training exception occurs through the previously failing third step. Step-2 compute time is `235.83s` versus `234.47s` without offload; step 3 takes `250.73s`, with a remaining ETA near 7-8 hours. Native log: `torchrun_logs/training_20260907_013012_datava270000004.log`; detached session: `vc3-v29-clean-20260907`. The 21-test vision suite passes, including nested-wrapper HF loading and CUDA/BF16 forward/gradient parity.
...
- Data: v26's seed-42 12,624-row TimeLens random-half manifest over 8,985 videos. Build a separate mode-aware cache; visual placeholder counts and timestamp strings must match v26's spatial-quarter query layout. Expected packing is 4,401 packs / 276 optimizer steps.
- Trainable scope: `416,870,640` original ViT, `143,887,104` LACT-added parameters, and `33,039,616` projector parameters (`593,797,360` total); LM frozen.
...
- Expected artifact: `/mnt/localssd/VideoChat3/training/vc3-lact-l16-delta-3drope-parallel-gate0-chunktailr4-vitfwproj-timelens-r12624-8xh100-gb16-f448-s4k-lr2e5-v32/20260907043817/hf-276`.
...
- Expected artifact: `/mnt/localssd/VideoChat3/training/vc3-base-chunkuniformr4-vitproj-timelens-r12624-8xh100-gb16-f448-s4k-lr2e5-v34/20260907051526/hf-276`, exported as a Base `videochat3_macro` model with matching uniform-selection processor metadata.
...
- Data: pinned LLaVA-Video-178K revision `6d8c562dc26d70042a0d9704d1cae58c94b89098`, Academic + YouTube 0–30s open/multiple-choice QA. Full prepared train manifest `LLaVA_Video_0_30s_train_VideoChat3.json` under `/mnt/localssd/dataset/VideoChat3/LLaVA-Video-178K`: 495,013 conversations / 2,242,912 QA turns, 87,787 clips / 82,918 source videos. The 44,957 held-out questions and separate captions are excluded. Source-ID overlap with heldout and the recorded benchmark exclusion set is zero.
- Trainable scope: original ViT, all LACT FW/gates, and multimodal projector; Qwen3 LM frozen. No learned query parameters.
- Vision/output: parallel Linear16+Delta, group 1, fast-Q/K 3D RoPE, apply-then-update with final update skipped. Preserve original four-frame attention, temporal-mean/2x2 spatial patch merger, every merged token, and timestamps. No extra compression or selection; a 224x224, 64-frame video gives 1,024 visual tokens. Training occurrences average 14.9995 FW updates per layer.
- Optimizer/LR schedule: AdamW, uniform ViT/FW/projector LR, weight decay 0, one epoch, cosine `2e-5 -> 1e-6`, 3% warmup (154 of 5,158 expected steps), inner Delta write strength `0.01`, no gate-specific LR. HF and DCP saves every 200 steps and at normal completion, keeping the latest of each.
- Stabilization: no NS5 and no FW ratio clipping; global gradient clip 1.0, full BPTT, ordinary FSDP activation checkpointing, CPU activation offload disabled.
- Hardware/batch/sequence: 4xH100 on physical GPUs 4–7; GPUs 0–3 are occupied by an unrelated job and are not used. Global batch 16 (four accumulations per rank), 8K sample/pack lengths, 2 FPS with 64-frame min/max (limited by source frames), 224px frame-pixel cap and 3,211,264 total-pixel budget. Seed-42 native packing gives 634,111,947 tokens, 82,518 packs, and 5,158 steps. No GPU-killing watchdog.
- Training W&B: [v35](https://wandb.ai/LVSM-Experiment/videochat3/runs/vc3-lact-l16-delta-3drope-parallel-alltokens-vitfwproj-llava0to30-qa495013-4xh100-gb16-f64-s8k-lr2e5-v35).
- Launcher: `xtuner-videochat3/training_scripts/stage3/VideoChat3_4B_LACT_ALLTOKENS_train_llava_0_30s_v35.sh`.
...
**Status:** Running on public W&B on physical GPUs 4–7, launched 2026-09-10; startup validated through step 3/417. Initialization is the requested v35 `hf-800`, not smoke weights.
...
- Expected artifact: `/mnt/localssd/VideoChat3/training/vc3-lact-l16-delta-parallel-3drope-multi_stage_video_last-timelens-r12624-hf800-4xh100-gb16-s8k-v36/20260910041658/hf-417` (final native `video_last`). First stage-boundary export is `hf-53`.
...
Formal startup: runtime reproduces 12,624 rows / 6,658 fixed packs, with ViT/FW/projector trainable, LM frozen, and stage 1 scheduled for steps 1–53. Steps 1–3 global CE is `0.24821888/0.30457830/0.27718985`, with finite pre-clip norms `15.39843273/18.80107689/14.53774357`; maximum rank allocation/reservation is `19.91/21.09 GB`, with no OOM, skipped update, or token mismatch. Steps 2–3 take `49.94/41.00s`. The W&B API confirms `running` with uploaded loss, gradient, and curriculum-stage metrics under the existing public account. Native log: `torchrun_logs/training_20260910_041644_datava270000004.log`; detached session: `vc3-v36-20260910`; executable launch commit: `869fcdf`. `video_curriculum.json` in the run directory records the stage boundaries used by the live trainer.
