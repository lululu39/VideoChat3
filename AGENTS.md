...
- There is no semantics-free long-chain fix. TBPTT or `stop_state` preserves forward/inference state but removes long-range credit assignment; per-state backward clipping keeps a bounded but biased gradient; first-order modes detach update gradients; changing NS5 coefficients, normalization, head count, or update scale changes the FW update model. Keep these as explicit ablations rather than silently altering VideoLACT math.
