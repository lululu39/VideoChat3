...
- Launch the 8xH100 Stage 3 vision-only run with `training_scripts/stage3/VideoChat3_4B_LACT_VE_train_stage3.sh`. It uses the retained LACT init plus the 17-dataset local lightweight manifest and logs rank-zero metrics to the public `LVSM-Experiment/videochat3` W&B project as `vc3-4b-lact-fw4-ve-s3-lite-8xh100-gb16-v1`, with JSONL retained on every rank. The single-node global batch is 16 (2 packs/rank) and the linearly scaled ViT LR is `6.25e-7` to bound long-video prefetch memory.
