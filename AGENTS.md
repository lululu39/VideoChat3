...
- v4 is the first LongVid experiment. It restarts from the retained LACT initialization, trains only the 358.47M LACT-added parameters with the v2 LR recipe (`2e-5 -> 1e-6`), and keeps both NS5 and complete state-adjoint ratio clipping enabled. The active public W&B run is `vc3-4b-lact-fw4-fwonly-longvid5870-8xh100-gb16-img1fps-f512-s8k-fwlr2e5-ns5r1-stgr1-v4` under `LVSM-Experiment/videochat3`; startup produced 2,141 packs / 134 steps, and step 1 completed cleanly with global calibrated CE `2.79301`, rank-0 pre-all-reduce diagnostic `loss` `0.31798`, pre-clip grad norm `3.072`, and a roughly 9h26m initial ETA.
