...
- The first full-local f128 run is diagnostic only: persistent pre-clip norms in the hundreds with spikes above one million make its checkpoints unsuitable as research results. Before restarting, log separate FW/base-ViT parameter norms plus per-update state adjoints and test stability at 8/16/32 groups.
