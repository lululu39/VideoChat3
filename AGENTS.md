...
- Launch the 8xH100 Stage 3 vision-only run with `training_scripts/stage3/VideoChat3_4B_LACT_VE_train_stage3.sh`; the launcher rebuilds/validates the full-local annotations first. It uses the retained LACT init and logs rank-zero metrics to public W&B `LVSM-Experiment/videochat3` as `vc3-4b-lact-fw4-ve-s3-full89k-8xh100-gb16-f128-s8k-vitlr2p5e6-ns5r1-v1`, with JSONL on every rank. The run explicitly enables NS5 ratio clipping at rho 1 and uses global batch 16 (2 packs/rank), 8K packs, at most 128 frames/video and 128x224x224 total video pixels. ViT LR `2.5e-6` is the common linear scaling of Stage 1 (`4e-5` at GB256) and Stage 2 (`2e-5` at GB128), with `lr_min=1.25e-7`.
...
- The current 128-frame limit is a project-added diagnostic cap, not an official Stage 3 setting; official Stage 3 defaults to as many as 3,600 sampled frames. At four frames per FW group, the diagnostic run can unroll 32 groups / 31 effective updates per layer, while the full official limit could reach 900 groups / 899 updates.
...
- The first full-local f128 run used exact/unclipped NS5 backward and is diagnostic only: it was stopped after step 1050 and its W&B run is tagged accordingly. Persistent pre-clip norms in the hundreds with spikes above one million make its checkpoints unsuitable as research results; the clean rho-1 comparison restarts from the retained LACT initialization rather than resuming those weights.
