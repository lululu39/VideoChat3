...
- `clip_ns_grad_ratio=True` is now the default LACT vision setting. It preserves the exact NS5 forward/inference path but customizes each independent matrix VJP so `||dG||_F / ||dU||_F <= 1`; set it to `False` for the original exact backward. A fixed tiny scan reduced the 32-group FW gradient from `1.11e4` to `4.15e-4` and kept 8/16/32-group gradients flat, but this constrains only the local NS5 backward, not the complete state-transition Jacobian.
