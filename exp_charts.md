...
本表汇总所有使用 TimeLens-100K 训练的 canonical 编号实验 `v9-v34`，并加入未训练 Base R4 作为固定参考。除注明外，训练数据均为 seed-42 的 12,624-row random-half manifest，评测均为同一 TimeLens-Bench native generation/scoring 协议（2 FPS、最多 448 帧、224px/14,680,064 total pixels）。内部系统 smoke 合并在对应版本中，不单列为学习实验。
...
| v33 | Stopped, no ckpt | V+F+P | 每 chunk 均匀选 `floor(S/4)` 个原始空间 token | 用户在 step 26 停止并转向 Base v34；末20步 CE `0.2946`，无原生评测 | `26/276`, 4K | `0.277829@26` | `13.010 / 12.549 / 24.302` | — | — | — | — |
| v34 Base | Running | V+P | 每 chunk 均匀选 `floor(S/4)` 个原始空间 token | 无 FW/query；与 v33 token 数逐位一致、前两步 CE 精确复现，启动约9s/step | `3/276`（启动记录）, 4K | `0.588378@3` | `19.034 / 15.154 / 28.203`（steps 1-3） | — | — | — | — |
